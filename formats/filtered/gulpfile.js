...
gulp.task('buildExternalJS', function () {
	var config = {
		...
		'ext.srf.filtered.select.js': ['node_modules/select2/dist/js/select2.js'],
	};
...
gulp.task('copyExternalImages', function () {
	var config = {
		'ext.srf.filtered.leaflet.css': ['node_modules/leaflet/dist/images/*'],
	...
	for (var target in config) {
		ret =
			ret && gulp.src(config[target]).pipe(gulp.dest('resources/css/images'));
	}
...
gulp.task('watch', function () {
	gulp.watch('resources/ts/**/*.ts', gulp.series('buildFiltered'));
	gulp.watch('tests/qunit/**/*.ts', gulp.series('buildFilteredTests'));
});

gulp.task(
	'buildExternalCSS',
	gulp.parallel('buildLeafletCSS', 'buildSliderCSS', 'buildSelectCSS'),
);
gulp.task(
	'default',
	gulp.parallel(
