...
( function ( $, mw ) {
	...
	$.fn.slideshow = function ( options ) {
		const results = options.data[ 0 ];
		const template = options.data[ 1 ];
		const delay = options.data[ 2 ];
		const height = options.data[ 3 ];
		const width = options.data[ 4 ];
		const navControls = options.data[ 5 ];
		const effect = options.data[ 6 ];
		const printrequests = options.data[ 7 ];
		...
		function fetchResult( index, moveForward, speed ) {
			...
			$.ajax( {
				...
				success: function ( result ) {
					...
					const wrapper = $( '<div class="slideshow-element-wrapper">' );
		...
		function switchTo( index, moveForward, speed ) {
			let animateForward, animateBackward;
			...
			if ( results[ index ].length === 1 ) {
				...
				if ( typeof results[ index ][ 1 ] !== 'boolean' ) {
					// find next result
					const newWrapper = results[ index ][ 1 ];
					...
					const oldWrapper = targetDiv.children();
		...
		if ( navControls ) {
			...
			mw.loader.using( 'jquery.ui', () => {
				const readout = $( '<div class="slideshow-nav-readout">' + 1 + '</div>' );
				...
				nav.wrap( "<div class='slideshow-nav-wrapper'></div>" );
