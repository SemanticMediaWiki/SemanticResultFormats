...
( function ( $, srf ) {
	...
	const methods = {
		...
		init: function () {
			...
			/**
			 * @member plotClass identifies class that holds the plot
			 * @member addedHeight collects heights of objects other that the chart in order
			...
			// Set-up chart title
			const chartTitle = data.parameters.charttitle;
			...
			// Set-up zoom box
			const zoom = '<div class="' + plotClass + '-zoom" style="height:50px"></div>';
			...
			const _tabs = chart.find( '.ui-tabs-nav' );
			if ( _tabs.length > 0 ) {
		...
		chart: function ( content ) {
			const plotArea = this.find( '.' + content.plotclass ),
			...
			function convertData( tseries ) {
				let ttData = [];
				let max = 0;
				const len = tseries.length;
				for ( let j = 0; j < len; ++j ) {
					ttData = tseries[ j ].data;
					for ( let p = 0; p < ttData.length; ++p ) {
			...
			// Draw actual plot
			let plot = $.plot( plotArea, data, options );

			if ( content.parameters.zoom === 'top' || content.parameters.zoom === 'bottom' ) {

				// Init zoom box
				const zoombox = $.plot( plotZoomArea, data, {
					series: {
				...
				this.bind( 'plotselected', ( event, ranges ) => {
				...
				plotZoomArea.bind( 'plotselected', ( event, ranges ) => {
			...
			const b = function ( i ) {
				...
				h = function ( i ) {
					const l = i.getUTCFullYear() + '-' + b( i.getUTCMonth() + 1 );
					return l + '-' + b( i.getUTCDate() );
				};

			let previousPoint = null;

			$( '#' + content.chartid ).bind( 'plothover', ( event, pos, item ) => {
