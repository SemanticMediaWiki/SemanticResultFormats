...
( function ( $ ) {
	...
	// Fallback for legacy browsers without a global console
	if ( typeof console === 'undefined' ) {
		// eslint-disable-next-line no-global-assign -- intentional polyfill of the global console
	...
	$.fn.srfjqPlotBubbleChartData = function ( options ) {
		...
		const dataRenderer = function () {
			...
			if ( errMsg === '' ) {
				for ( let k = 0; k < data.data[ 0 ].length; ++k ) {
		...
		const jqplotbubbledata = dataRenderer();
	...
	$.fn.srfjqPlotBubbleChart = function ( options ) {
		...
		const jqplotbubble = $.jqplot( options.id, options.barData, {
