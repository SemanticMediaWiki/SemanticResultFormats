...
( function ( $ ) {
	...
	$.fn.srfjqPlotBarChart = function ( options ) {
		...
		const numberaxis = {
			...
			autoscale: !!( data.parameters.stackseries || data.parameters.autoscale ),
			padMax: 0,
			padMin: 0,
			tickOptions: {
		...
		const base = Math.pow( 1, Math.floor( Math.log( max( labels ), 10 ) ) );
		...
		const highlighter = {
			show: !!( data.parameters.highlighter && data.renderer === 'line' ),
		...
		$.jqplot.LabelFormatter = function ( format, val ) {
			const num = typeof val === 'object' ? val[ 1 ] : val;
		...
		// Color information
		const seriesColors = data.parameters.seriescolors ? data.parameters.seriescolors : data.parameters.colorscheme === null ? null : colorscheme[ data.parameters.colorscheme ][ 9 ];

		// Enable jqplot plugins
		$.jqplot.config.enablePlugins = true;

		// Now we are plotting
		const jqplotbar = $.jqplot( options.id, options.barData, {
