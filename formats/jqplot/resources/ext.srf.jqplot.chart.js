...
( function ( $, srf ) {
	...
	function _getElementAttribute( options ) {
		const p = $( '<p></p>' ).hide().appendTo( options.context ).addClass( options.className ),
	...
	$.fn.srfjqPlotChartContainer = function () {
		...
		const adjustChartElement = 'srf-jqplot-chart-adjust ' + data.parameters.gridview + ' ' + profile.name,
		...
		// Plotting area
		const plotID = chartID + '-plot';
		container.prepend( '<div id="' + plotID + '" class="srf-jqplot-plot"></div>' );
		const plot = chart.find( '.srf-jqplot-plot' );
		plot
