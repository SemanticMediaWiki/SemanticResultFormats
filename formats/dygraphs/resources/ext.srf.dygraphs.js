...
( function ( $, mw, srf ) {
	 ...
	 * @return void
	 */
	const _createTooltip = function ( options ) {
		const typeMap = { warning: '4', error: '5', info: '1' };
		const $anchor = $( '<span>' )
	...
	$.fn.extend( {
		srfdygraphs: function ( options ) {
			return this.each( function () {
				...
				/**
				 * @member plotClass identifies the class that holds the plot
				 * @member plotID identifies the ID that holds the plot mainly used by dygraphs
				 *
				 * Instances
				 *
				...
				function getData( options ) {
					...
					const jqxhr = $.ajax( {
						...
						.fail( ( error ) => {
							...
							const responseText = error.responseText !== '' ? ' (' + error.responseText + ')' : '';
				...
				function initGridView() {
					...
					if ( data.data.source.annotation !== undefined ) {
						$.map( data.data.source.annotation, ( val ) => {
				...
				// Slice from position 1 because the 0 is the x-value descriptor wich is not needed here
					const seriesItem = _addCheckboxItem( {
				...
				function initGraph( options ) {
					...
					const g = new Dygraph(
						...
						( () => options.data ), {
							...
							showRoller: data.parameters.rollerperiod > 0,
							...
							drawCallback: function ( g, is_initial ) {
								if ( !is_initial ) {
									...
									// jquery ui tabs
									const tabsHeight = chart.find( '.ui-tabs-nav' ).outerHeight();
									plotInstance.css( { height: plotInstance.height() - tabsHeight } );
									g.resize();

									// Create and display annotations
									const annotations = getAnnotations( data.data.source.annotation );
									if ( annotations !== undefined ) {
