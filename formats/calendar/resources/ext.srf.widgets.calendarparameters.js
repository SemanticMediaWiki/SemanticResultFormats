...
( function ( $, mw, srf ) {
	...
	$.widget( 'srf.calendarparameters', {
		...
		dateSelection: function ( options ) {
			...
			self.calendar.datepicker( {
				...
				onSelect: function ( dateText ) {
					...
					const fromProperty = options.list[ $( '#mini-calendar-from', self.input ).data( 'property' ) ],
			...
			self.input.on( 'click', 'input:radio[name=option], .reset-link', function ( event ) {
				const fromDate = $( '#mini-calendar-from', self.input ).val(),
		...
		colorFilter: function ( options ) {
			...
			self.filterparam.on( 'change', '#filterproperty, #legend, #filter', ( event ) => {
				const propertyIndex = $( '#filterproperty', self.filterparam ).val(),
