...
class SRFWord extends FileExportPrinter {
	...
	private function readValue(/* SMWDataValue */ $dataValue, $plabel) {
		...
		if ($l_isimage) {
			// has the PhpWord TemplateProcessore been patched?
			if (method_exists($this->document,"searchImageId")) {
				...
				wfDebug("SRF_Word image handling only availabe with patched PhpWord Templateprocessor");
			}
		} else {
