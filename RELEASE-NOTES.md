...
* Fixed `gallery` format showing a raw `<span class="smw-localized-message">` marker instead of the resolved "further results" link label under SMW 7.0+: the link was built in `SMW_OUTPUT_HTML` mode, which HTML-escapes SMW's deferred-localized-message marker (meant to survive as unescaped markup and be resolved post-cache); now built in wiki mode like SMW core's `list`/`table`/`category` printers and parsed to HTML, matching how the format's own `isHTML` output requires real markup ([1058](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1058)) (by [gesinn.it](https://gesinn.it))
