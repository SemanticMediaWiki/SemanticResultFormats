...
* Wired the vendored `resources/jquery/fullcalendar/fullcalendar.js` (v3.9.0) into the node-qunit environment, closing the coverage gap noted above: `fullCalendar(...).init()` now runs the real jQuery plugin against jsdom (using the npm `moment` package to satisfy its internal `require('moment')`, since the vendored `moment.js` there is a browser-global build with no CommonJS export), and gained a test asserting one rendered event anchor per input event with none nested inside another, plus a test covering `eventRender`'s icon-insertion for multiple events. While investigating, found `fullCalendar(...).event()`'s `icon()`/`description()` looking for `.fc-event-time`/`.fc-event-title`, which fullCalendar 3.9.0 never renders (it emits `.fc-time`/`.fc-title`) — both branches were consequently dead code, silently never inserting the configured event icon or the inline/tooltip description; fixed the selectors and updated the existing `fullCalendar(...).event()` tests' fixtures to match. `fullCalendar(...).update()`/`.resize()` and the document-ready block remain uncovered. Separately, while reproducing [1135](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1135) (events rendering nested inside one another in an `agendaDay` view with several adjacent events), found that fullCalendar 3.9.0's own time-grid event-stacking layout depends on real pixel geometry (`getBoundingClientRect()`/`offsetHeight`), which jsdom always reports as 0 — the exact nesting from that issue could not be reproduced under jsdom for this reason, and would need a real-browser test harness (by [gesinn.it](https://gesinn.it))
