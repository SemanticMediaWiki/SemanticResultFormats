...
* Added a jQuery UI widget-factory shim (`jquery-ui`'s `widget`/`mouse`/`slider` modules, plus a `widgetBaseClass` restoration matching MediaWiki's bundled `jquery.ui.widget.js`) to the node-qunit environment, and ported the `optionslist`, `panel`, and `parameters` widget legacy browser-QUnit tests; fixed several `assert.ok(jQueryObject)` no-op assertions along the way, which had been masking that `srf.panel`'s generated element is a sibling (not a descendant) of its context and that `srf.parameters`' `_init()` creates no wrapper element (by [gesinn.it](https://gesinn.it))
* Wired Leaflet (plus `leaflet.markercluster`/`leaflet-providers`) into the node-qunit environment and ported the `filtered` format's `DistanceFilter` test (expanded to exercise the haversine distance calculation) and `MapView`'s `init()`-level (marker/bounds-building) test; `MapView`'s `show()`/`lateInit()`-exercising tests stay in the legacy browser-QUnit suite, which needs a real Leaflet map render and `window.matchMedia` (by [gesinn.it](https://gesinn.it))
* Ported the `gallery` format's `init`, `overlay`, and `redirect` legacy browser-QUnit tests, the `media` format's `init`/`template`/`parse`/`getData` tests (rewriting its `defaults` test, which previously hung on a failed/404 request instead of failing cleanly), and the `tagcloud` format's `init`/`dependencies`/`load` tests to the node-qunit runner; `tagcloud`'s `sphere`/`wordcloud` tests stay legacy, needing real tagcanvas/d3 canvas/SVG rendering (by [gesinn.it](https://gesinn.it))
* Rewrote the `datatables` format's node-qunit test, which had been calling `init()` with a stale 3-argument signature (the real method takes 2) against fixture data missing fields the current `init()` unconditionally reads — both defects were invisible since the test was never wired into CI; the rewrite exercises the real DataTables jQuery plugin end to end (by [gesinn.it](https://gesinn.it))
* Ported the `eventcalendar` format's `parse.api()` and `startDate()` (self-contained query-result-to-calendar-event transformation) legacy browser-QUnit tests to the node-qunit runner; the rest of the legacy suite stays, including `update()`, which uses the QUnit 1.x `stop()`/`start()` API removed in QUnit 2.x and cannot run in any modern QUnit runtime (by [gesinn.it](https://gesinn.it))
* Documented the remaining intentionally-legacy QUnit test files (calendar widgets, `eventcalendar`'s `update` sub-test, `MapView`'s `show()`/`lateInit()` tests, `tagcloud`'s `sphere`/`wordcloud` tests) with a `tests/qunit/README.md` explaining why each only runs manually via `Special:JavaScriptTest/qunit` (by [gesinn.it](https://gesinn.it))
...
* Added ESLint (`eslint-config-wikimedia`), scoped to our own JS code (`resources/`, `formats/*/resources/`, `tests/node-qunit/`) and excluding bundled third-party libraries and the legacy browser-only QUnit suite via `.eslintignore`; fixed the majority of resulting findings (933 → 376 across ~65 files), mostly formatting, missing MediaWiki/vendor globals, `var`→`let`/`const`, `eqeqeq`, and stricter QUnit assertions; remaining findings are tracked as follow-up issues (by [gesinn.it](https://gesinn.it))
* Fixed the `resources/` ESLint findings (38 errors across 9 files): removed redundant `/* global mw semanticFormats mediaWiki */` comments that redeclared already-configured globals, removed dead code (`ext.srf.api.query.js`'s unused `results` instance, `ext.srf.api.results.js`'s unused `timeLocales` object, unused `srf` widget-factory parameters and `el` locals in the widget files), replaced string concatenation of literals and `Object.prototype.hasOwnProperty()` direct calls, renamed `ext.srf.util.js`'s `_CL_mwspinner`/`_CL_srfIspinner`/`_CL_srfspinner` constants to camelCase, and switched several never-reassigned `let`s to `const` ([1079](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1079)) (by [gesinn.it](https://gesinn.it))
* Disabled ESLint's `mediawiki/valid-package-file-require` rule for `tests/node-qunit/**/*.js`: the rule's dot-prefix heuristic misidentifies the `.compiled/` directory (used for TypeScript-compiled `filtered` format test doubles) as already dot-relative and suggests stripping the required `./` prefix, which breaks `require()` resolution under the suite's `NODE_PATH` setup — confirmed by re-running `npm run node-qunit` (69/69) after the rule's autofix regressed 6 tests ([1080](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1080)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/calendar` ESLint findings (52 errors across 5 files): removed a redundant `/* global mw smw mediaWiki semanticMediawiki semanticFormats */` comment and unused `srf` widget-factory parameters, removed dead code (unused `metaData`/`prevElement`/`self`/`el`/`content` locals and unused callback parameters), replaced string concatenation of literals and `Object.prototype.hasOwnProperty()` direct calls with safe equivalents, replaced a ternary used only for its side effect with `if`/`else`, hoisted `EventCalendar`'s `calendar` instance above its first use to resolve `no-use-before-define`, and fixed loose (`==`) equality checks, keeping one intentional bitmask check with a targeted `no-bitwise` disable ([1081](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1081)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/jqplot` ESLint findings (15 errors across 3 files): removed redundant `/* global mw colorscheme simple vector */` comments that redeclared already-configured globals, switched several never-reassigned `let`s to `const`, merged a string concatenation of adjacent literals, and disabled `no-new` for a grid-widget instantiation kept for its side effect ([1082](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1082)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/boilerplate` ESLint findings (6 errors across 2 files): removed redundant `/* global mediaWiki semanticFormats */`/`/* global mw */` comments that redeclared already-configured globals, hoisted `ext.srf.boilerplate.namespace.js`'s `util` instance above its first use to resolve `no-use-before-define`, and replaced dead commented-out `console.log( data )` example calls with real `srf.log( data )`/`mw.log( data )` calls to resolve `no-unused-vars` ([1083](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1083)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/d3` ESLint findings (20 errors across 2 files): trimmed `/* global d3:true, mw:true, colorscheme:true */` comments to `d3` only, since `mw` and `colorscheme` are already configured globals, hoisted each file's `util` instance above its first use to resolve `no-use-before-define`, and switched several never-reassigned `let`s to `const` ([1084](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1084)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/gallery` ESLint findings (5 errors across 2 files): switched never-reassigned `let`s to `const` and merged a string concatenation of adjacent literals in `ext.srf.gallery.overlay.js`, and fixed a loose (`==`) equality check in `ext.srf.gallery.slideshow.js`; added node-qunit coverage for the fixed `overlay()` title-formatting and `slideshow()` preceding-`<br>` branches ([1085](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1085)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/widget` ESLint findings (9 errors across 2 files): removed a redundant `/* global mw:true */` comment that redeclared an already-configured global and disabled `camelcase` for the third-party `pajinate` plugin's fixed snake_case option keys in `ext.srf.listwidget.js`, and fixed four loose (`==`) equality checks in `ext.srf.pagewidget.carousel.js` ([1086](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1086)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/Gantt` ESLint findings (8 errors in `ext.gantt.js`): removed unused `index`/`value` callback parameters, switched never-reassigned `let`s to `const`, replaced `Object.prototype.hasOwnProperty()` direct calls and loose (`==`/`!=`) equality checks with safe/strict equivalents in `forceTextWrappingOn()`'s SVG text-wrapping logic ([1087](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1087)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/datatables` ESLint findings (13 errors in `ext.srf.formats.datatables.js`): removed an unused `html` local and unused trailing callback/function parameters (`obj`, `rowIdx`, `self`, `settings`), switched never-reassigned `let`s to `const`, replaced five `console.log()` debug calls with `mw.log()`/`mw.log.error()`, and fixed a loose (`!=`) equality check in `showNotice()`; added node-qunit coverage for the `init()` Ajax code path (cache-miss → `mw.Api().post()` → `mw.log`/`mw.log.error`), which was previously untested ([1088](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1088)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/dygraphs` ESLint findings (18 errors in `ext.srf.dygraphs.js`): removed a redundant `/* global mw:true Dygraph:true mediaWiki:true */` comment that redeclared already-configured globals, removed the unused `_addIcon` helper and unused function parameters (the plugin's own `options`, `getData`'s `complete`, `prepareContainer`'s `options`, `underlayCallback`'s `g`), switched never-reassigned `let`s to `const`, merged a string concatenation of adjacent literals, renamed `line_delimiter`/`is_initial` to camelCase, and disabled `no-new` for a grid-widget instantiation kept for its side effect ([1089](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1089)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/media` ESLint finding (1 error in `ext.srf.formats.media.js`): disabled `no-new` for the `jPlayerPlaylist` instantiation kept for its side effect, instead of assigning it to an unused `jPlayerInstance` local ([1090](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1090)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/sparkline` ESLint findings (4 errors in `ext.srf.sparkline.js`): removed a redundant `/* global mw:true semanticFormats:true */` comment that redeclared already-configured globals, hoisted the `util` instance above its first use to resolve `no-use-before-define`, and disabled `no-new` for the `srf.formats.sparkline` instantiation kept for its side effect ([1091](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1091)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/tagcloud` ESLint findings (4 `no-bitwise` errors in `ext.srf.formats.tagcloud.js`): replaced `~~x` bitwise-truncation with `Math.floor(x)` in `wordcloud()`'s random rotation/font selection ([1092](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1092)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/timeline` ESLint findings (30 errors in `ext.srf.timeline.js`): renamed `smw_make_timeline`/`smw_get_bandwidth`/`smw_add_event` to camelCase, removed an unused `l` local, split ten one-line band-type `switch`/`case` statements onto separate lines, and fixed sixteen loose (`==`) equality checks against DOM properties (`className`, `nodeType`) and loop counters ([1093](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1093)) (by [gesinn.it](https://gesinn.it))
* Fixed the `formats/timeseries` ESLint findings (5 errors in `ext.srf.timeseries.flot.js`): removed a redundant `/* global mw:true */` comment that redeclared an already-configured global, switched a never-reassigned `let` to `const`, disabled `no-new` for the `srf.util.grid` instantiation kept for its side effect, and fixed two loose (`==`) equality checks against `undefined` in the zoom-selection y-range calculation ([1094](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1094)) (by [gesinn.it](https://gesinn.it))
* Enabled ESLint in CI now that all follow-up lint issues (#1079–#1094) are resolved: `npm test` now runs `npm run lint` before `node-qunit`, so a future ESLint error fails the build instead of only showing up via the standalone `npm run lint` script ([1095](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1095)) (by [gesinn.it](https://gesinn.it))
* Fixed the JS coverage report covering almost none of the codebase: nyc's `include` glob (`formats/**/resources/js/*.js`) only matched the 3 `filtered` module files, while the 34 other format JS files tested via node-qunit live directly under `formats/*/resources/*.js` and were silently excluded, reporting 0% coverage; broadened `include` to `formats/**/*.js` and `resources/**/*.js` (mirroring `.eslintignore`'s exclusions for vendored/minified/generated files in `nyc.exclude`), raising real coverage to ~18% of statements across 44 files ([1095](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1095)) (by [gesinn.it](https://gesinn.it))
* Fixed PHPCS scanning `formats/filtered/node_modules/nouislider/concat.php` (a bundled npm dependency, not our code), which broke `make ci` with a hard error; added `node_modules` to `.phpcs.xml`'s exclude-pattern alongside the existing `vendor`/`conf` exclusions ([1095](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1095)) (by [gesinn.it](https://gesinn.it))
* Added node-qunit coverage for `formats/slideshow/resources/ext.srf.slideshow.js` (previously 0%, now ~81% of statements): fetching/displaying a result and its API-error path, preloading the next result, delay-based auto-advance (including wraparound and the no-delay case), all seven transition effects, and the nav-controls slider/pause-button wiring, including its Enter-key handler and the auto-init loop over the `srf_slideshow` PHP-generated global; also added `mw.util.wikiScript()` to the shared node-qunit MediaWiki mock, which this file needed and no prior test exercised ([1099](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1099)) (by [gesinn.it](https://gesinn.it))
* Added node-qunit coverage for `formats/timeline/resources/ext.srf.timeline.js` (previously 0%, now ~85% of statements), driven through a minimal fake of the vendored SimileTimeline `Timeline` global against real `SRF_Timeline.php`-shaped markup: the document-ready `.smwtimeline` initialization loop, band creation from `smwtlband` markers (including the documented 1–4 band width split and the default single-band fallback), `smwtlposition`-derived band positioning, and event parsing from `smwtlstart`/`smwtlend`/`smwtlurl`/`smwtltitle`/`smwtlprefix`/`smwtlpostfix`/`smwtlcoloricon` markers; also pinned (without fixing) a pre-existing bug where the default single-band fallback omits the `date` option, silently ignoring a `smwtlposition` marker whenever no `timelinebands` are configured, filed as [1114](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1114) ([1100](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1100)) (by [gesinn.it](https://gesinn.it))
* Added node-qunit coverage for the `srf.calendarparameters` jQuery UI widget in `formats/calendar/resources/ext.srf.widgets.calendarparameters.js` (previously 0%, now ~97% of statements): the mini-calendar date-range picker (`dateSelection`, including its `onSelect`/`gotoDate`/`onReset` callbacks and printout-dropdown wiring), the `limit` slider and its `_limitConstrain()` value adjustment, the `eventStart` earliest/latest radio portlet, the `colorFilter` legend/filter portlet, and the `option`/`destroy` entry points; fixed a bug found while testing `destroy( { class } )`, which read the widget's never-assigned `this.pane` property and so removed same-class elements anywhere in the document instead of only within the widget instance — changed it to search `this.element` like every other method in the file ([1102](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1102)) (by [gesinn.it](https://gesinn.it))
* Added node-qunit coverage for the `srf.calendarlegend` jQuery UI widget in `formats/calendar/resources/ext.srf.widgets.calendarlegend.js` (previously 0%, now ~84% of statements): the `top`/`bottom`/`pane` legend-container placement, the deduplicated colour-square rendering with its default-colour fallback, the `filter`-type checkbox rendering and its click-driven `onFilter` callback, the empty-list hide/show toggle, and the `option`/`destroy` entry points; the `tooltip` position branch (`_refreshTooltip()`) is left uncovered, needing a `calendarbutton`-compatible DOM fixture and the real SMW-core `smw.util.tooltip` behaviour, neither available to the node-qunit environment. Fixed two bugs found while testing: `_create()` passed its bare `list` option straight to `_refreshList()`, which reads its argument as `{ list, type }` (as `_setOption()` correctly does) — so the legend never rendered on initial widget creation and only ever appeared after a later `option( 'list', { list, type } )` update; and `destroy( { class } )` had the same never-assigned `this.pane` scoping bug fixed in `calendarparameters` above, changed to `this.element` ([1107](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1107)) (by [gesinn.it](https://gesinn.it))
* Added node-qunit coverage for `formats/calendar/resources/ext.srf.formats.eventcalendar.js` (previously ~21% of statements, now ~47%): `parse.api()`'s remaining branches (the `includeend` date-only-vs-time-bearing end-date shift, `icon`/`color`/`iconclass`/wikiPage-and-plain-object `filterProperty` printouts, and the show/hide-headers description-collection paths), `getTruncatedSentence()`, `defaults.set()`'s theme/view-name/firstday/holiday derivation, `getID()`, `onDayClick()`'s clicktarget URL templating (including its hour-24 same-day special case and Moment.js-object date handling), and `fullCalendar(...).event()`'s icon-insertion and description tooltip/inline-span branches; exposed these previously-private methods via the existing `test` interface, added static `wgArticlePath`/`wgServer` `mw.config` defaults and a `confirm()`/`window.open()` shim to the node-qunit environment (jsdom does not leak these onto the Node global scope the way a real browser would), and gave the mocked `smw.util.tooltip` a `.show()` no-op so it can be stubbed like the real API. Left uncovered: `init()`/`update()`, `fullCalendar(...).init()`/`.update()`/`.resize()`, and the document-ready block, none of which are reachable without a real fullCalendar jQuery plugin instance. Noted, but not fixed, a dead condition in `parse.api()`'s legend-building: `color` is set via `'color' in rowData ? rowData.color : null`, so it is never `undefined`, making the surrounding `color !== undefined` check always true and `null` colours indistinguishable from real ones in the legend's collected `color` array ([1111](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1111)) (by [gesinn.it](https://gesinn.it))
* Added node-qunit coverage for `resources/ext.srf.api.query.js` (previously 6.2% of statements, now ~98%): `conditions.build()`'s default-vs-explicit operator, `printouts.toList()`'s plain-vs-identified (`|?property=identifier`) printout splitting, `printouts.search.identifier()`, `printouts.search.type()`'s type-matching and identifier-exclusion filtering (used by `eventcalendar` to build its date-property and colour-filter lists), `toString()`'s condition/printout/parameter concatenation, and `fetch()`'s `$.ajax()` request shape and success/failure callback wiring, including its `log`-flag branch. Fixed a bug found while testing `printouts.search.type()`: `ext.srf.api.results.js`'s `getTypeId()` threw a `TypeError` instead of returning `null` whenever asked for a property not present in the current printrequests list — always the case for a printout carrying a custom identifier (`|?property=identifier`), since the identifier (not the property label) was looked up in a map keyed by label; this made `search.type()` throw for any query mixing identified and non-identified printouts of the same type, rather than filtering the identified one out as documented ([1113](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1113)) (by [gesinn.it](https://gesinn.it))

* Wired the vendored `resources/jquery/fullcalendar/fullcalendar.js` (v3.9.0) into the node-qunit environment, closing the coverage gap noted above: `fullCalendar(...).init()` now runs the real jQuery plugin against jsdom (Node's `require('moment')`, resolved from the new `moment` devDependency, satisfies fullcalendar.js's internal `require("moment")` call — the vendored `moment.js` there is a browser-global build with no CommonJS export). Fixed three defects this surfaced, all pre-existing and not caused by prior refactoring:
** [1135](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1135) (events rendering nested inside one another in an `agendaDay`/`agendaWeek` view with several adjacent events): fullCalendar 3.9.0 builds one HTML string per time-grid column covering all of that column's event segments, then splits it back into per-segment DOM nodes with `$(html)` and matches each node back to its segment *by array index*. Its segment template closed a `<div class="fc-bg"/>` with self-closing syntax; jQuery ≥3.5 dropped the legacy `rxhtmlTag` normalisation that used to expand this to a real closing tag, so jQuery's HTML parser folded every subsequent segment as a *descendant* of that div instead of a sibling — collapsing N segments into 2 top-level nodes regardless of N. With exactly 1 event this desynced the index entirely (`Cannot read properties of undefined (reading 'footprint')`, calendar failed to render at all); with 3+ it produced the nested-`.fc-bg` structure from the reported issue. Fixed the 4 self-closing `<div .../>` instances in the affected segment-template code (fullcalendar.js's vendored source, not our own code) to real open/close tags.
** `fullCalendar(...).event()`'s `icon()` looked for `.fc-event-time`/`.fc-event-title`, which fullCalendar 3.9.0 never renders (it emits `.fc-time`/`.fc-title`) — the icon insertion was consequently dead code, never actually placing the configured event icon in any view. Fixed the selector. `description()`'s tooltip rendering was already reachable and correct in every view; its dead inline-text branch (guarded by the same wrong selector) is left as-is rather than newly enabled, since doing so would silently switch existing wikis' `agendaDay`/`basicDay` views from a tooltip to inline text and drop the tooltip's description-length truncation — an intentional behaviour change, not a bug fix, and out of scope here.
** Neither insertion had any stylesheet rule: an unstyled `<img>` icon distorted event row height, and `.srf-fc-description` ran on against the event title with no separator. Added `.fc-event img`/`.srf-fc-description` rules to `ext.srf.formats.eventcalendar.css`.
* Fixed [1138](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1138) (`Uncaught TypeError: calendar.init is not a function`, breaking every `eventcalendar` render): a prior ESLint fix had hoisted `const calendar = new srf.formats.eventcalendar();` above the `srf.formats.eventcalendar.prototype = { ... }` assignment to resolve a `no-use-before-define` warning — but that assignment *replaces* the prototype object, so the already-constructed `calendar` instance kept pointing at the original empty one and never gained `.init`. Changed the assignment to `Object.assign( srf.formats.eventcalendar.prototype, { ... } )`, which mutates the existing prototype object in place instead of replacing it, fixing the bug without reintroducing the lint warning. Added a regression test that drives the module's actual `$(document).ready()` handler (the only thing that calls `calendar.init()` on a real page) — every other test in the file constructs `new srf.formats.eventcalendar()` after the module has fully evaluated, so none of them observed the broken intermediate prototype state, which is why 241/241 tests stayed green while the format was completely broken on every real wiki.
* All of the above was additionally verified against a live MediaWiki 1.43 + SemanticMediaWiki `dev-master` + SRF stack in a real browser (Playwright/Chromium), not just jsdom: three adjacent events in an `agendaDay` view now render as three independent, non-nested DOM elements with no console errors, matching the node-qunit results (by [gesinn.it](https://gesinn.it))

### Maintenance

* Updated the `build/` docker-compose-ci submodule to pick up `extensions.local.json` support for non-bundled custom extensions and SMW 7 compatibility (skips the now-removed `ConfigPreloader`/`enableSemantics()` calls, silencing a deprecation notice on every test run); added `extensions.local.json` to install the `Diagrams` extension and `graphviz` (providing the `dot` binary it renders locally with, no network access required) to `OS_PACKAGES`, so CI can exercise the `graph` format's `Diagrams`-specific code path (`ExtensionRegistry::isLoaded( 'Diagrams' )` branches in `GraphFormatter`/`GraphPrinter`) instead of only the untested fallback; fixed `GraphFormatterTest::testGetWordWrappedText`, which had been implicitly assuming Diagrams is never loaded (by [gesinn.it](https://gesinn.it))
* Added the `ExternalData` extension (pinned to `3.6.1`, the latest release compatible with the CI matrix's MediaWiki 1.43–1.46) to `extensions.local.json`, downloaded but not `wfLoadExtension`'d by default to avoid colliding with `Diagrams`' competing `<graphviz>` tag handler — both extensions call `Parser::setHook( 'graphviz', ... )` and MediaWiki keeps only the last registration. Loaded per-test instead, via an ad-hoc autoloader, by the new `GraphvizRenderingTest` integration suite's end-to-end smoke test of External Data's `<graphviz>` tag-emulation mode (by [gesinn.it](https://gesinn.it))
* Added `tests/phpunit/Integration/Graph/GraphvizRenderingTest.php`: integration tests for the `graph` format's `<br />`-vs-newline line-break fix ([816](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/816)) that render through the real `dot` binary (no mocking), covering all three extensions capable of rendering a `graph` format's DOT output (GraphViz/`dot` directly, Diagrams, and External Data's `<graphviz>` tag-emulation mode) (by [gesinn.it](https://gesinn.it))
...
* Roughly halved the `filtered` format's client-side configuration payload (the `srfFilteredConfig` JS variable) by compacting its per-item serialization: printouts are now a positional array of `{ v, f, s }` slots (with the formatted `f` and sort `s` values omitted when identical to the plain `v` values, and value-less printouts stored as `null`) instead of a per-item map keyed by a repeated hash, and geographic coordinates are single-sourced instead of duplicated per row. Gadgets or user scripts that read `srfFilteredConfig` directly need to be updated to the new structure (by [Professional Wiki](https://professional.wiki))
...
* Added a `map view layers` parameter to the `filtered` format's map view, giving the map multiple base layers with a Leaflet layer switcher. Each name is either a leaflet-providers provider (e.g. `OpenStreetMap.Mapnik`) or a custom base layer defined by the Maps extension via `$egMapsLeafletLayerDefinitions` (Maps 13.1+); the first name is the initially active layer. When the parameter is unset, the map keeps its previous single-provider behaviour (by [Professional Wiki](https://professional.wiki))
* Added a `map view geojson` parameter to the `filtered` format's map view, drawing a GeoJSON overlay (from a `GeoJson:` page or a URL, fetched by the Maps extension) on top of the result markers. The overlay honours the mapbox simplestyle-spec (stroke and fill styling) and `title`/`description` popups, is toggleable from the layer switcher, and is unaffected by the filters (by [Professional Wiki](https://professional.wiki))
...
* Added `GraphPrinterTest` coverage for the `graph` format's display parameters (`graphname`, `graphfontsize`, `graphsize`, `nodeshape`, `arrowdirection`, `graphlink`, `nodelabel`, `graphlabel`, `graphcolor`, `arrowhead`, `relation`, `graphlegend`), each asserted against the actual DOT source (`GraphFormatter::getGraph()`) the `<graphviz>` tag/`dot` binary receives, through the real `GraphPrinter` → `GraphFormatter` pipeline (by [gesinn.it](https://gesinn.it))

### Bug Fixes

* Fixed `filtered` format crashing with "Call to a member function getMaxIncludeSize() on null" when a filter parameter (e.g. `value filter switches`) was parsed via `Parser::recursiveTagParse()` outside a regular page parse, such as `Special:Ask` or the REST API: `Filtered::getParser()` now calls `Parser::startExternalParse()` with `ParserOptions::newFromContext()` before returning the shared parser service if its `ParserOptions` are unset ([802](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/802)) (by [gesinn.it](https://gesinn.it))
* Fixed `gallery` format showing a raw `<span class="smw-localized-message">` marker instead of the resolved "further results" link label under SMW 7.0+, and a related stored-XSS regression in the first fix attempt: the link is now built in wiki mode (like SMW core's `list`/`table`/`category` printers) and resolved to HTML via SMW's `RecursiveTextProcessor`, which safely falls back to a full parse when none is in progress — unlike an earlier `isSpecialPage()`-guarded approach, which left an anonymous, unauthenticated `action=purge` request (where no parse is running) able to splice a user-supplied `searchlabel` into the page as raw, unparsed wikitext, persisted in the parser cache ([1058](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1058), [1136](https://github.com/SemanticMediaWiki/SemanticResultFormats/pull/1136)) (by [gesinn.it](https://gesinn.it))
...
* Fixed `graph` format showing a literal `<br />` instead of a line break in node labels wrapped under the Diagrams extension: a fieldless node's label was re-escaped with a second `htmlspecialchars()` pass and rendered as a quoted DOT string label, neither of which lets GraphViz interpret `<br />` as a line break; now rendered as an HTML label instead, matching how the `graphfields` branch already handles it ([846](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/846)) (by [gesinn.it](https://gesinn.it))
...
* Fixed `filtered` format `DistanceFilter` throwing `TypeError` when a result row has no data for the distance-filtered property, instead of treating it as infinitely far away (by [gesinn.it](https://gesinn.it))
...
* Fixed CI: Codecov upload only ever contained JS coverage — coverage files land in `build/coverage/` (Docker volume mounted via `build/docker-compose-ci.yml`), not at the repo root, so the `codecov-action`'s `coverage/php/coverage.xml,coverage/js/clover.xml` paths didn't resolve; Codecov fell back to auto-discovery, which only picks up the JS `clover.xml` (repo-relative paths) and silently drops the PHP report (`/var/www/html/...`-prefixed paths, which need `codecov.yml`'s `fixes:` rewrite — only applied to explicitly-passed files, never to auto-discovered ones). Corrected the paths to `build/coverage/php/coverage.xml,build/coverage/js/clover.xml` (by [gesinn.it](https://gesinn.it))
...
* Fixed `graph` format `GraphPrinter::processResultRow()` creating a node from a page-type value that should have been skipped (`$pageTypeSeen > 1`, i.e. from the 2nd page-type printout onward): the `elseif ( $showAsEdge )` branch duplicated the primary node-creation check without its `!$skipNode` guard, so a 2nd-or-later page-type printout without a property could still become the node. Removed the duplicated check instead of patching it, since the primary check earlier in the same loop iteration already governs node creation by the time this branch runs — discovered via new test coverage for ([1124](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1124)) (by [gesinn.it](https://gesinn.it))
* Fixed `graph` format silently dropping the entire row (node, edges, and fields) for queries with an explicit subject column (`?=`) at the 2nd-or-later page-type printout position, and, conversely, creating a bogus node from an unrelated earlier page-type value instead of the query subject: node identity in `GraphPrinter::processResultRow()` was inferred from column position (`$pageTypeSeen`) rather than from the query subject (`PrintRequest::PRINT_THIS`). The `?=` printout is now always treated as the row's node regardless of its position among other page-type printouts (by [gesinn.it](https://gesinn.it))
* Fixed `graph` format losing an edge whenever its node candidate appeared after the edge's own column (e.g. a `?=` subject column placed after other printouts): `GraphPrinter::processResultRow()` built edges/fields inline while iterating the row, so the node was still unset while an earlier column's edge was being decided. Split into reading every value once, determining the node from the full row, then collecting edges/fields against the already-known node (by [gesinn.it](https://gesinn.it))
* Fixed `graph` format ignoring an explicitly suppressed printout label (e.g. `?Property=`): `GraphPrinter::readRowValues()` fell back from the intentionally empty label to the canonical property name via `?:`, and `GraphNode::addField()` plus the DOT field-table row in `GraphFormatter::buildGraph()` had the same fallback, so the property name (and a stray `: ` prefix) still showed up even after suppressing it. All three now only fall back when the value is genuinely unset (`null`), not merely an empty string ([1131](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1131)) (by [gesinn.it](https://gesinn.it))
* Fixed `graph` format field values (`graphfields=yes`) wrapping with a plain newline instead of `<br />` whenever the Diagrams extension wasn't loaded: field values (and the fields-table header) always render inside a `<td>` of a DOT HTML label, where only a literal `<br />` is interpreted as a line break — a plain newline is silently swallowed by `dot`, running wrapped words together. The wrap separator no longer depends on `ExtensionRegistry::isLoaded('Diagrams')` for this code path; verified against the real `dot` binary and, for External Data's `<graphviz>` tag-emulation mode, via a full `Parser::parse()` end-to-end test. As a side effect, the fields-table header now always wraps too (previously only under `nodelabel=displaytitle`; a fieldless node's own label is unaffected and keeps that gating, since it can render as either a quoted DOT string or an HTML label depending on content — see [846](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/846)), which can change node geometry on upgrade for existing `graphfields=yes` wikis with long captions. While refactoring, also removed a now-dead `str_replace( '<br />', '', $nodeTooltip )` call: the caption used to derive the tooltip is only ever wrapped (and thus could ever contain a literal `<br />` to strip) when `nodelabel=displaytitle`, but the header/tooltip source is now always the caption's raw, unwrapped value ([816](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/816)) (by [gesinn.it](https://gesinn.it))
* Fixed `graph` format fields-table header rendering empty for a single-character caption or node ID (e.g. element/gene-symbol-style page titles): `GraphFormatter::getWordWrappedText()`'s regex silently dropped any token too short to satisfy either alternative (the "at least `$charLimit` characters" branch, or the "at least 2 characters, followed by whitespace or end-of-string" branch) instead of emitting it verbatim; added a final `|\S+` alternative to catch it (by [gesinn.it](https://gesinn.it))
* Fixed `filtered` format crashing with "Multiple conflicting values given for srfFilteredConfig" as soon as a page had more than one filtered query: `Filtered::addConfigToOutput()` passed an ever-growing, per-query config map to `ParserOutput::setJsConfigVar()`, which requires the exact same value on every call — and each query's client-side ID is freshly random, so two calls within the same parse could never match. That call is now removed; `Hooks::onOutputPageParserOutput()` delivers the config to `mw.config` via `OutputPage::addJsConfigVars()` instead, which merges safely and has no such constraint ([1039](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/1039), [1052](https://github.com/SemanticMediaWiki/SemanticResultFormats/pull/1052)) (by @thomas-topway-it)
* Fixed `filtered` format ignoring a page's Display Title in filter widgets when the query used `link=none`: `ResultItem::getArrayRepresentation()` only ever passed the result of `getShortHTMLText( $linker )` as the client's "formatted" value, and SMW core's `WikiPageValue::getShortHTMLText( null )` never consults Display Title in its no-linker branch. The formatted value now resolves `WikiPageValue::getDisplayTitle()` directly whenever no linker is available and a Display Title is set, independent of the `link` parameter ([561](https://github.com/SemanticMediaWiki/SemanticResultFormats/issues/561)) (by [gesinn.it](https://gesinn.it))
