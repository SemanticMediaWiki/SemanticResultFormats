...
QUnit.module('ext.srf.widgets.parameters', () => {
	...
	QUnit.test('instance', (assert) => {
		...
		assert.ok(result.is(context), 'the srf.parameters widget returned the context element (its _init() creates no wrapper)');
	});
	...
	QUnit.test('limit parameter test', (assert) => {
		...
		parameters.parameters('limit', {
			...
			step: 1,
			change: (event, ui) => {
				assert.equal(ui.value, 3 - 1, 'the limit parameter was changed to 2');
			},
		});

		assert.equal(parameters.find('.value').text(), '10', 'the limit parameter is 10');
		...
		assert.equal(parameters.find('.value').text(), '3', 'the limit parameter is 3');
	});
