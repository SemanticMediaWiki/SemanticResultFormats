...
const { ViewSelector } = require( './.compiled/Filtered/ViewSelector.js' );
...
QUnit.module( 'ext.srf.filtered ViewSelector', () => {
	...
	QUnit.test( 'can construct', ( assert ) => {
		...
		assert.true( v instanceof ViewSelector, 'can construct ViewSelector' );
	} );
	...
	QUnit.test( 'init for 1 view', ( assert ) => {
		...
		assert.true( target.is( ':hidden' ), 'target element is NOT visible' );
	...
	QUnit.test( 'init for 2 views', ( assert ) => {
		...
		assert.true( target.children().first().hasClass( 'selected' ), 'first view selector is marked as selected' );
	...
	QUnit.test( 'selecting views when clicked (3 views: foo, bar, baz)', ( assert ) => {
		...
		c.onViewSelected = ( viewID ) => {
			assert.true( true, `Controller was called to select view "${ viewID }"` );
		};
