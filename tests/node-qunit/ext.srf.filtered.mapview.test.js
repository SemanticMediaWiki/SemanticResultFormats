...
const { MapView } = require('./.compiled/Filtered/View/MapView');
...
QUnit.module('ext.srf.filtered MapView', () => {
	...
	QUnit.test('init builds one marker per position and extends the bounds', (assert) => {
		...
		mapView.init().then(() => {
			...
			assert.ok(mapView.bounds !== undefined, 'bounds were computed from the marker positions');
			done();
		}).catch((e) => {
			assert.ok(false, `init() rejected: ${e.message}`);
			done();
		});
	});
	...
	QUnit.test('init skips rows that have no data for this view\'s id', (assert) => {
		...
		const done = assert.async();
		mapView.init().then(() => {
			assert.ok(Object.prototype.hasOwnProperty.call(mapView.markers, 'row1'), 'row1 (has matching data) got an entry');
			assert.notOk(Object.prototype.hasOwnProperty.call(mapView.markers, 'row2'), 'row2 (no matching data) got no entry');
			done();
		}).catch((e) => {
			assert.ok(false, `init() rejected: ${e.message}`);
			done();
