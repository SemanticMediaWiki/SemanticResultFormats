...
QUnit.module('ext.srf.formats.media', () => {
	...
	QUnit.test('defaults', (assert) => {
		...
		$.get(media.defaults.posterImage).done(() => {
			assert.ok(getStub.calledWith(media.defaults.posterImage), media.defaults.posterImage + ' verified');
			done1();
		});

		const done2 = assert.async();
		$.get(media.defaults.jplayer.swfPath).done(() => {
			assert.ok(getStub.calledWith(media.defaults.jplayer.swfPath), media.defaults.jplayer.swfPath + ' verified');
			done2();
		});
	});
