...
const { MockedFilter } = require( './ext.srf.filtered.mocked-filter.js' );
...
QUnit.module( 'ext.srf.filtered Controller', () => {
	...
	QUnit.test( 'can construct and attach data', ( assert ) => {
		...
		assert.true( c instanceof Controller, 'can construct Controller' );
	...
	QUnit.test( 'attaching 3 views (foo, bar, baz) and switching between them', ( assert ) => {
		...
		for ( const viewId in views ) {
			...
			assert.true(
				viewsShown.length === 1 && viewsShown.includes( views[ viewId ] ),
	...
	QUnit.test( 'show', ( assert ) => {
		...
		assert.true( targetShown, 'container made visible' );
	} );

	// done() is only wired to the resolution path (pre-existing); attachFilter()
	// rejecting would leave the test hanging until timeout rather than calling
	// done(), same as before this pass
	...
	QUnit.test( 'attaching 3 filters (foo, bar, baz)', ( assert ) => {
		...
		filterIds.forEach( ( filterId ) => {
			...
			const promise = controller.attachFilter( filter ).then( () => {
				assert.true( visibilityWasQueried, `filter "${ filterId }" was queried after attaching` );
			} );
