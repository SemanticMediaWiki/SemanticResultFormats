...
QUnit.module( 'ext.srf.formats.tagcloud', () => {
	...
	QUnit.test( 'init', ( assert ) => {
		...
		assert.strictEqual( $.type( tagcloud.defaults ), 'object', '.defaults was accessible' );
		assert.strictEqual( $.type( tagcloud.sphere ), 'function', '.sphere() was accessible' );
		assert.strictEqual( $.type( tagcloud.wordcloud ), 'function', '.wordcloud() was accessible' );
		assert.strictEqual( $.type( tagcloud.load ), 'function', '.load() was accessible' );
	} );

	QUnit.test( 'dependencies', ( assert ) => {
		const util = new srf.util();

		assert.strictEqual( $.type( util.assert ), 'function', 'util.assert was accessible' );
		assert.strictEqual( $.type( smw.async.load ), 'function', 'smw.async.load was accessible' );
		assert.strictEqual( $.type( util.spinner.hide ), 'function', 'util.spinner.hide was accessible' );
		assert.strictEqual( $.type( util.message.set ), 'function', 'util.message.set was accessible' );
	} );
	...
	QUnit.test( 'load', ( assert ) => {
		...
		assert.true( result, 'sphere was initialized' );
		...
		result = tagcloud.load( options );
		assert.true( result, 'wordcloud was initialized' );
		...
		result = tagcloud.load( options );
		assert.true( result, 'non existing element' );
		...
		result = tagcloud.load( options );
		assert.strictEqual( result, false, 'wrong version' );
	} );
