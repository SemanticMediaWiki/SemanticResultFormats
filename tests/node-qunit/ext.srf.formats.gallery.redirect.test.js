...
QUnit.module( 'ext.srf.formats.gallery.redirect', {
	...
	QUnit.test( 'redirect handles mw-file-description selector', ( assert ) => {
		...
		assert.strictEqual( imageLink.attr( 'href' ), 'http://example.com/redirect-target', 'Image href updated to redirect URL from alt attribute' );
		assert.notStrictEqual( redirectIcon.css( 'display' ), 'none', 'Redirect icon is visible for URI type redirect' );
	} );
	...
	QUnit.test( 'redirect resolves article titles', ( assert ) => {
		...
		setTimeout( () => {
			...
			assert.true( imageLink.length > 0, 'Found image link with mw-file-description class' );
			assert.strictEqual( imageLink.attr( 'href' ), '/wiki/Valid_Title', 'Image href updated to resolved article URL' );
			assert.notStrictEqual( redirectIcon.css( 'display' ), 'none', 'Redirect icon is visible after successful title resolution' );
			done();
		}, 100 );
	} );
	...
	QUnit.test( 'redirect handles failed title resolution', ( assert ) => {
		...
		setTimeout( () => {
			...
			assert.strictEqual( imageLink.attr( 'href' ), '', 'Image href cleared on failed title resolution' );
			assert.true( redirectIcon.is( ':hidden' ), 'Redirect icon hidden on failed title resolution' );
			done();
		}, 100 );
	} );
