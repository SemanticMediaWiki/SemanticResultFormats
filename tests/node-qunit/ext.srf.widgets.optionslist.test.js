...
QUnit.module('ext.srf.widgets.optionslist', () => {
	...
	QUnit.test('instance', (assert) => {
		...
		assert.ok(result.length > 0, 'the srf.checklist widget returned an object with length > 0');
	});
	...
	QUnit.test('checklist click event', (assert) => {
		...
		let result = context.optionslist('checklist', {
			...
			class: 'test',
			click: (event, ui) => {
				assert.equal(ui.value, 11, 'object key (11) was returned for id(#bar)');
			},
		});
		...
		result = context.optionslist('checklist', {
			...
			class: 'test',
			click: (event, ui) => {
				assert.equal(ui.value, 1, 'array key (1) was returned for id(#bar)');
			},
		});
	...
	QUnit.test('checklist show/hide', (assert) => {
		...
		assert.notEqual(result.css('display'), 'none', 'show: true resulted in a visible list');
		...
		assert.notEqual(result.css('display'), 'none', 'omitting show resulted in a visible list (defaults to shown)');
		...
		assert.equal(result.css('display'), 'none', 'show: false resulted in a hidden list');
	});
	...
	QUnit.test('selectlist change event', (assert) => {
		...
		let result = context.optionslist('selectlist', {
			...
			class: 'test',
			change: (event, ui) => {
				assert.equal(ui.value, 11, 'value (11) was returned');
			},
		});
		...
		result = context.optionslist('selectlist', {
			...
			class: 'test2',
			change: (event, ui) => {
				assert.equal(ui.value, 2, 'value (2) was returned');
			},
		});
